...
fun get(@PathVariable @Size(max = MAX_ID) id: String): OrderResponse {
    requireIdWithinLimit(id)  // 実行されない。ここのエラーメッセージも使われない
...
宣言的検証の違反例外は、自前の例外階層の外からフレームワークが投げる。ドメイン例外の変換を整備していても、この例外の変換が漏れると意図したステータスではなくフレームワーク既定の応答（500等）になる。変換の配置は「例外変換のスコープ」に従う。違反時に投げられる例外の型はフレームワークの構成やバージョンに依存するため推測せず、統合テストで実際に飛ぶ例外とステータス・応答形状を固定する。

| 基準 | 判定 |
|------|------|
| 同じ制約が宣言的検証と手続き的チェックの両方に実装されている | REJECT。宣言側に一本化し手続き側を削除 |
| 宣言的検証の違反例外が変換されずフレームワーク既定の応答（500等）になる | REJECT |
| 検証違反時のステータスと応答形状を固定するテストがない | REJECT。実際に飛ぶ例外型を統合テストで確認 |
| 入口の種類（ボディ・パス・クエリ）で検証スタイルが混在している | 警告。宣言的スタイルに揃える |
