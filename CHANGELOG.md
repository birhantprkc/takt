...
- **BREAKING:** `provider: auto` was removed (#1040). Automatic routing is now enabled by the presence of effective `auto_routing` candidates while the top-level `provider` stays concrete. Operations without workflow-step context (such as AI task-slug generation) use the concrete top-level provider/model; `auto_routing.router` and candidates are never implicit defaults. Configs that set `provider: auto` must switch the top level to a concrete provider — routing behavior is otherwise unchanged.
